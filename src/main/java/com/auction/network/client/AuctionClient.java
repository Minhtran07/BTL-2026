...
public class AuctionClient {
     ...
     * Dùng {@link AtomicReference} để thao tác thread-safe:
     * thread sendRequest set, thread listener complete.
     */
    private final AtomicReference<CompletableFuture<Message>> pendingResponse =
    ...
    /**
     * Gửi 1 request lên server và đợi response.
     *
    ...
    private void listenForMessages() {
        while (connected) {
            try {
                ...
                if (type == Message.Type.AUCTION_EVENT || type == Message.Type.BID_UPDATE) {
                    ...
                    CompletableFuture<Message> future = pendingResponse.get();
                    if (future != null) {
        ...
        CompletableFuture<Message> pending = pendingResponse.getAndSet(null);
        if (pending != null) {
